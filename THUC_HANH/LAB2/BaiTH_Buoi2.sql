...
from employees
where
    hire_date between to_date('20/02/1998', 'dd/mm/yyyy') and to_date('01/05/1998', 'dd/mm/yyyy')
...
select
    initcap(last_name) as "ten nhan vien",
    length(last_name) as "chieu dai"
from employees
where
    substr(last_name, 1, 1) in ('J', 'A', 'L', 'M')
order by last_name asc;
...
select
    ...
    ) as "so thang lam viec"
from employees
order by months_between (sysdate, hire_date) asc;

select
    last_name || ' earns ' || to_char(salary, '$99,999') || ' monthly but wants ' || to_char(salary * 3, '$99,999') as "dream salaries"
from employees;
...
select job_id, decode(
        job_id, 'AD_PRES', 'A', 'ST_MAN', 'B', 'IT_PROG', 'C', 'SA_REP', 'D', 'ST_CLERK', 'E', '0'
    ) as "grade"
from employees;
...
select
    ...
    sum(
        case
            when to_char(hire_date, 'yyyy') = '1995' then 1
            else 0
        end
    ) as "nam 1995",
    sum(
        case
            when to_char(hire_date, 'yyyy') = '1996' then 1
            else 0
        end
    ) as "nam 1996",
    sum(
        case
            when to_char(hire_date, 'yyyy') = '1997' then 1
            else 0
        end
    ) as "nam 1997",
    sum(
        case
            when to_char(hire_date, 'yyyy') = '1998' then 1
            else 0
        end
    ) as "nam 1998"
from employees;
